...
public interface WeightRepository extends JpaRepository<Weight, Long> {
    @Query("select weight from Weight weight where weight.userweight.login = ?#{principal.username}")
    List<Weight> findByUserweightIsCurrentUser();
    ...
    @Query(
        value = "select distinct weight from Weight weight left join fetch weight.userweight",
        countQuery = "select count(distinct weight) from Weight weight"
    )
    Page<Weight> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct weight from Weight weight left join fetch weight.userweight")
    List<Weight> findAllWithToOneRelationships();

    @Query("select weight from Weight weight left join fetch weight.userweight where weight.id =:id")
