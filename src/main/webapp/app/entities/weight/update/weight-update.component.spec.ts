...
describe('Weight Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call User query and add missing value', () => {
      ...
      const userweight: IUser = { id: 19261 };
      weight.userweight = userweight;

      const userCollection: IUser[] = [{ id: 48569 }];
      jest.spyOn(userService, 'query').mockReturnValue(of(new HttpResponse({ body: userCollection })));
      const additionalUsers = [userweight];
    ...
    it('Should update editForm', () => {
      const weight: IWeight = { id: 456 };
      const userweight: IUser = { id: 30575 };
      weight.userweight = userweight;

      activatedRoute.data = of({ weight });
      comp.ngOnInit();

      expect(comp.usersSharedCollection).toContain(userweight);
